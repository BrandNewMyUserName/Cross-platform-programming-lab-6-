...
describe('IrrigationSystem', () => {
  ...
  it('should throw an error if coverageArea is not a positive number', () => {
    expect(() => new IrrigationSystem(1, 'SmartSprinkler', 89.99, 'USA', -100)).toThrow(new Error('Площа покриття має бути додатньою.'));
    expect(() => new IrrigationSystem(1, 'SmartSprinkler', 89.99, 'USA', 0)).toThrow(new Error('Площа покриття має бути додатньою.'));
  });

  it('should throw an error if id is not a positive integer', () => {
    expect(() => new IrrigationSystem(-1, 'SmartSprinkler', 89.99, 'USA', 100)).toThrow(new Error('ID має бути додатнім цілим числом.'));
  });

  it('should throw an error if name is empty', () => {
    expect(() => new IrrigationSystem(1, '', 89.99, 'USA', 100)).toThrow(new Error('Назва товару не може бути порожньою.'));
  });

  it('should throw an error if price is not a positive number', () => {
    expect(() => new IrrigationSystem(1, 'SmartSprinkler', -89.99, 'USA', 100)).toThrow(new Error('Ціна має бути додатнім числом.'));
  });

  it('should throw an error if country is empty', () => {
    expect(() => new IrrigationSystem(1, 'SmartSprinkler', 89.99, '', 100)).toThrow(new Error('Країна виробника не може бути порожньою.'));
  });
});